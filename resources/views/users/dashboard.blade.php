<x-layout>
    ...
    {{-- User Posts --}}
    <h2 class="mb-4 font-bold">Your Latest Posts</h2>
            ...
            <x-postCard :post="$post">
                ...
                <div class="flex items-center justify-end gap-4 mt-6">
                    ...
                    <a href="{{ route('posts.edit', $post) }}"
                        class="px-2 py-1 text-xs text-white bg-green-500 rounded-md">Update</a>

                    {{-- Delete post --}}
                    <form action="{{ route('posts.destroy', $post) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button class="px-2 py-1 text-xs text-white bg-red-500 rounded-md">Delete</button>
                    </form>
                </div>
            </x-postCard>
        @endforeach
